...
async function main() {
    ...
    const {vault, weth, dai, router, user0, positionManager, mlpManager, usdm} = await getContracts();

    // let amount = await vault.poolAmounts(dai.address)
    // console.log("amount:", formatEther(amount));
    
    // let getAum = await mlpManager.getAum(true);
    // console.log("getAum:", formatUnits(getAum, 30));
    
    // let getAum = await mlpManager.getAumInUsdm(false);
    // console.log("getAum:", formatUnits(getAum, 30));
    
    // let cooldownDuration = await mlpManager.cooldownDuration();
    // console.log("cooldownDuration:", cooldownDuration);

    // await usdm.addVault(mlpManager.address);
    
    // await mlpManager.setCooldownDuration(300);
